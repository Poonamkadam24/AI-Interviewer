    ...
    try:
        ...
        if isinstance(evaluation_data, dict):
            ...
            for k, v in evaluation_data.items():
                ...
                if k_norm in ["technicalscore", "techscore", "score"]:
                    try:
                        normalized["technicalScore"] = int(v)
                    except (ValueError, TypeError):
                        normalized["technicalScore"] = 0
                elif k_norm in ["confidencescore", "confidence"]:
                    try:
                        normalized["confidenceScore"] = int(v)
                    except (ValueError, TypeError):
                        normalized["confidenceScore"] = 0
                elif k_norm in ["aifeedback", "feedback"]:
                    normalized["aiFeedback"] = str(v) if not isinstance(v, str) else v
                elif k_norm in ["idealanswer", "answer", "ideal"]:
                    normalized["idealAnswer"] = json.dumps(v) if isinstance(v, (dict, list)) else str(v)
                else:
                    normalized[k] = v

            return EvaluationResponse(
                technicalScore=normalized.get("technicalScore", 0),
                confidenceScore=normalized.get("confidenceScore", 0),
                aiFeedback=normalized.get("aiFeedback", "Evaluation completed."),
                idealAnswer=normalized.get("idealAnswer", "No ideal answer provided.")
            )
        else:
            return EvaluationResponse(technicalScore=0, confidenceScore=0, aiFeedback="Invalid JSON format received", idealAnswer="Invalid JSON format received")
